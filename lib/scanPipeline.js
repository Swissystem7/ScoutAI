 ...
 * @property {Object} [offBallSignals]
 */
...
function prepare(input, index, opts) {
  ...
  const actionValue = measuredActionValue(input);
  // Measured xT replaces the inferred momentum proxy when present. For an xT-only
  // record it is the complete primary score; with other stats it remains the
  // load-bearing 70% share rather than being diluted by inferred inputs.
  const baseStatValue = statResult ? statResult.score : (statScore === null ? 0 : statScore);
  const measuredValue = actionValue === null ? baseStatValue : (statResult && statResult.confidence > 0 ? round2(actionValue * 0.7 + baseStatValue * 0.3) : actionValue);
  const energy = face ? face.energy : videoEnergy(input.videoSignals);
  const inferredMomentum = actionValue === null ? (momentum === null ? videoMomentum(input.videoSignals) : momentum) : actionValue;
  ...
  const fused = actionValue !== null ? _fuse({
    stat: { value: measuredValue, confidence: 1 },
    genetic: null,
    narrative: null,
  }, false) : scientific || _fuse({
    stat: { value: measuredValue, confidence: statScore === null ? 0 : 1 },
    genetic: { value: heuristic.index, confidence: hasInferred(input) ? 0.65 : 0 },
    narrative: null,
  }, false);
  const primaryScore = round2(actionValue !== null ? clamp(fused.index, 0, 100) : scientific ? scientific.index : clamp(fused.index, 0, 100));
  const geometry = measuredGeometryChannel(input.videoSignals, input.offBallSignals);
  // All measured-geometry evidence shares one secondary 12% cap. The primary
  // channel retains 88%; scanning and persistence are deliberately absent.
  const indexScore = geometry.scored ? round2(primaryScore * 0.88 + geometry.score * 0.12) : primaryScore;
  geometry.contribution = geometry.scored ? round2(indexScore - primaryScore * 0.88) : 0;
  const conventional = conventionalValue(input, opts);
  return {
    id: input.id || slug(input.name) + '-' + index, name: input.name, source: input.source,
    minutes: input.minutes, age: input.age, position: input.position, statMetrics: input.statMetrics, provenance: input.provenance, missingSignals: input.missingSignals,
    ...
    contributions: {
      ...
      actionValue: actionValue === null ? null : { value: actionValue, contribution: round2(indexScore * statWeight(fused)), tag: 'measured', label: 'socceraction xT; measured action value' },
      scanning: geometry.body.present
        ? { value: geometry.body.scanningValue, contribution: 0, tag: 'unvalidated', unvalidated: true, label: 'excluded from scoring' }
        : { value: scan ? scan.score : null, tag: input.scanning && input.scanning.measured ? 'measured' : 'inferred' },
      bodyLanguage: geometry.body.present ? { value: geometry.body.score, contribution: geometry.body.scored ? geometry.contribution : 0, weight: geometry.body.scored ? 0.12 : 0, tag: 'measured-geometry', excluded: !geometry.body.scored, reason: geometry.body.scored ? null : 'pose confidence below 0.6', intensity: geometry.body.intensity, uprightness: geometry.body.uprightness, confidence: geometry.body.confidence } : null,
      offBall: geometry.offBall.present ? { value: geometry.offBall.score, contribution: geometry.offBall.scored ? geometry.contribution : 0, weight: geometry.offBall.scored ? 0.12 : 0, tag: 'measured-geometry', excluded: !geometry.offBall.scored, reason: geometry.offBall.scored ? null : 'comparison group below 8 players', offBallZ: geometry.offBall.offBallZ, closingZ: geometry.offBall.closingZ, persistence: { value: geometry.offBall.persistenceValue, contribution: 0, unvalidated: true } } : null,
      intangible: { value: actionValue === null ? heuristic.index : null, contribution: actionValue === null ? round2(indexScore * (1 - statWeight(fused))) : 0, tag: 'inferred', label: actionValue === null ? 'AI-inferred; secondary' : 'not used; measured xT available' },
    },
    diagnostics: { statSignature: statResult, scientific, impact, scanning: scan, faceEnergy: face, measuredGeometry: geometry, geneticPropensity: genetic, momentumShifts: shifts },
...
function scout(board, opts) {
  const candidates = board.filter(p => {
    const visibility = Number.isFinite(opts.maxMinutes) ? Number.isFinite(p.minutes) && p.minutes < opts.maxMinutes : p.lowVisibility;
    return visibility && (!Number.isFinite(opts.maxAge) || (Number.isFinite(p.age) && p.age <= opts.maxAge));
  });
  ...
  let outliers = ranked.map((r) => ({ ...byId.get(r.id), hiddenUpsideGap: r.upsideScore }));
  if (Number.isFinite(opts.maxMinutes)) outliers.sort((a, b) => b.index - a.index || a.id.localeCompare(b.id));
  outliers = outliers.map((r, i) => ({ ...r, scoutRank: i + 1 }));
  return { mode: 'scout', board, lowVisibilityCount: candidates.length, outliers, hiddenGem: outliers[0] || null, filter: { maxLeagueStrength: opts.maxLeagueStrength || 0.65, maxStatScore: opts.maxStatScore || 45, maxMinutes: Number.isFinite(opts.maxMinutes) ? opts.maxMinutes : null, maxAge: Number.isFinite(opts.maxAge) ? opts.maxAge : null } };
}
...
function measuredActionValue(p) {
  const m = p.statMetrics;
  if (!m || !m.actionValueProvenance || m.actionValueProvenance.measured !== true) return null;
  return Number.isFinite(m.actionValueScore) ? clamp(m.actionValueScore, 0, 100) : null;
}
function measuredBodyChannel(v) {
  if (!v || v.provenance !== 'measured-geometry') return { present: false, scored: false };
  const confidence = clamp(Number(v.confidence) || 0, 0, 1);
  const intensity = clamp(Number(v.intensity) || 0, 0, 1);
  const uprightness = clamp(Number(v.uprightness) || 0, 0, 1);
  const scanningValue = v.scanning && Number.isFinite(v.scanning.value) ? v.scanning.value : null;
  return {
    present: true,
    scored: confidence >= 0.6 && v.lowConfidence !== true,
    score: round2((intensity * 0.65 + uprightness * 0.35) * 100),
    intensity, uprightness, confidence, scanningValue,
  };
}
function measuredOffBallChannel(v) {
  if (!v || v.provenance !== 'measured-geometry') return { present: false, scored: false };
  const offBallZ = Number(v.offBallZ);
  const closingZ = Number(v.closingZ);
  if (!Number.isFinite(offBallZ) || !Number.isFinite(closingZ)) return { present: true, scored: false };
  // Convert standardized within-clip evidence to a bounded secondary score.
  const score = clamp(50 + 15 * (offBallZ * 0.65 + closingZ * 0.35), 0, 100);
  return {
    present: true, scored: v.lowConfidence !== true && Number(v.comparisonGroupSize) >= 8,
    score: round2(score), offBallZ, closingZ,
    persistenceValue: v.persistence && Number.isFinite(v.persistence.value) ? v.persistence.value : null,
  };
}
function measuredGeometryChannel(video, offBallSignals) {
  const body = measuredBodyChannel(video);
  const offBall = measuredOffBallChannel(offBallSignals);
  const scores = [body, offBall].filter(x => x.scored).map(x => x.score);
  const scored = scores.length > 0;
  const score = scored ? round2(scores.reduce((a, b) => a + b, 0) / scores.length) : 0;
  return { present: body.present || offBall.present, scored, score, body, offBall, contribution: scored ? null : 0 };
}
