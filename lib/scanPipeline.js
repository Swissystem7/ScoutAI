...
function prepare(input, index, opts) {
  ...
  const body = measuredBodyChannel(input.videoSignals);
  // Body geometry is secondary and capped at 12%. The primary measured/stat
  // channel always retains 88%; unvalidated scanning is deliberately absent.
  const indexScore = body.scored ? round2(primaryScore * 0.88 + body.score * 0.12) : primaryScore;
  ...
  return {
    ...
    contributions: {
      ...
      scanning: body.present
        ? { value: body.scanningValue, contribution: 0, tag: 'unvalidated', unvalidated: true, label: 'excluded from scoring' }
        : { value: scan ? scan.score : null, tag: input.scanning && input.scanning.measured ? 'measured' : 'inferred' },
      bodyLanguage: body.present ? { value: body.score, contribution: body.scored ? round2(indexScore - primaryScore * 0.88) : 0, weight: body.scored ? 0.12 : 0, tag: 'measured-geometry', excluded: !body.scored, reason: body.scored ? null : 'pose confidence below 0.6', intensity: body.intensity, uprightness: body.uprightness, confidence: body.confidence } : null,
    ...
    diagnostics: { statSignature: statResult, scientific, impact, scanning: scan, faceEnergy: face, bodyLanguage: body, geneticPropensity: genetic, momentumShifts: shifts },
